...
def build_portfolio_evaluation(advanced_results, target_return=0.15):
    """Tạo kết luận từ các kết quả đã tính, không gọi dữ liệu mới."""
    ...
    if pd.notna(oos_sharpe):
        ...
        stability_comment = f"Rolling OOS Sharpe trung vị {_fmt_num(oos_sharpe)}"
    else:
    ...
    if pd.notna(ir) and ir < 0:
        warnings.append("Danh mục đang kém hiệu quả so với VNINDEX sau khi điều chỉnh theo active risk.")
...
def render_advanced_section(advanced_results, title=None, target_return=0.15):
    """Hiển thị phân tích nâng cao và kết luận từ đúng kết quả run_research."""
    ...
    evaluation = build_portfolio_evaluation(advanced_results, target_return=target_return)

    st.markdown("---")
    st.markdown("**11.7. KẾT LUẬN ĐÁNH GIÁ DANH MỤC**")
