...
def factor_proxy_analysis(returns, company_table=None):
    """Factor analysis using only fields already available in the app.

    Proxies are intentionally derived from existing price and company data.
    No external factor series are queried.
    """
    ...
    for ticker in returns.columns:
        ...
        if company_table is not None and not company_table.empty and "Mã" in company_table.columns:
            ...
            if not row.empty and "Vốn hóa" in row.columns:
                liquidity = row.iloc[0].get("Vốn hóa", np.nan)
        quality = np.nan
        value = np.nan
        if company_table is not None and not company_table.empty and "Mã" in company_table.columns:
            row = company_table.loc[company_table["Mã"] == ticker]
            if not row.empty:
                if "P/B" in row.columns:
                    value = row.iloc[0].get("P/B", np.nan)
                if "ROE" in row.columns:
                    quality = row.iloc[0].get("ROE", np.nan)
        rows.append({
            "Mã": ticker,
            "Momentum 12T": trend,
            "Volatility": vol,
            "Size proxy vốn hóa": liquidity,
            "Value proxy P/B": value,
            "Quality proxy ROE": quality,
        })
    return pd.DataFrame(rows)


def multifactor_regression(portfolio_returns, factor_returns, rf=0.0):
    """OLS factor regression using an existing portfolio return series and supplied factor return series."""
...
def active_analysis(portfolio_returns, benchmark_returns, rf=0.0):
    ...
    return pd.DataFrame([{
        ...
        "Beta": p.cov(b) / b.var() if b.var() > 0 else np.nan,
        "Tương quan": p.corr(b),
    }])
...
def robustness_analysis(r, target_return=0.15, windows=(126, 252, 504)):
    ...
    for w in windows:
        ...
        rows.append({
            "Cửa sổ": w,
            "CAGR": _annualized_return(x),
            "Rủi ro": _volatility(x),
            "Sharpe": _sharpe(x, 0.0),
            "Max Drawdown": _max_drawdown(x),
            "Đạt mục tiêu": "Có" if _annualized_return(x) >= target_return else "Không",
        })
    return pd.DataFrame(rows)
...
def walk_forward_analysis(r, train_ratio=0.6, test_ratio=0.2, rf=0.0):
    ...
    while start + train + test <= n:
        ...
        rows.append({
            "Fold": fold,
            "Train CAGR": _annualized_return(train_r),
            "Train Sharpe": _sharpe(train_r, rf),
            "Test CAGR": _annualized_return(test_r),
            "Test Sharpe": _sharpe(test_r, rf),
            "Test Max Drawdown": _max_drawdown(test_r),
        })
        start += test
        fold += 1
    return pd.DataFrame(rows)
